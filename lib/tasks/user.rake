namespace :user do
  ...
  task crawl: :environment do
    ...
    Tasks::UserImporter.new.crawl_github_users(location: ENV['USER_LOCATION'])
  end
  ...
  task :geocode_locations, [:geocoder] => :environment do |t, args|
    ...
    User.select("id, location").where("(location IS NOT NULL) AND (location != '') AND ((city IS NULL) OR (city = ''))").each do |user|
